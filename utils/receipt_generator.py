...
def generate_receipt(order):
    ...
    story.append(Paragraph("Atendimento humanizado e respeitoso",
                           normal_style))
    ...
    order_data = [
        ...
        # --- MUDANÇA AQUI: Passamos o objeto 'order' inteiro para a função ---
        ['Forma de Pagamento:',
         get_payment_method_name(order)],
    ]

    # --- LÓGICA DE PARCELAS ATUALIZADA ---
    # Agora verifica se o método é 'credit' e se há mais de 1 parcela
    ...
    story.append(
        Paragraph("• Este comprovante serve como confirmação do seu pedido.",
                  normal_style))
    story.append(
        Paragraph("• Entraremos em contato para coordenar a entrega.",
                  normal_style))
    story.append(
        Paragraph(
            "• Para dúvidas, entre em contato pelo WhatsApp: (51) 98333-9080",
            normal_style))
    story.append(Spacer(1, 20))
...
def get_payment_method_name(order):
    """Converte o método de pagamento num nome legível, considerando as parcelas."""
    ...
    if method == 'credit':
        if installments > 1:
            return f'Cartão de Crédito Parcelado ({installments}x)'
        else:
